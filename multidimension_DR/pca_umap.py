...
# Function to perform PCA + UMAP and save results
def perform_pca_umap_on_shape(shape_folder, pca_dim, umap_dim):
    ...
    # Save PCA+UMAP results to CSV
    shape_name = os.path.basename(shape_folder)
    output_file = f"{shape_name}_pca-umap_{pca_dim}-{umap_dim}_output_{CURRENT_DATETIME}.csv"
    output_df = pd.DataFrame(reduced_data, columns=[f"dim{i + 1}" for i in range(umap_dim)])
    ...
    print(f"PCA+UMAP results saved to {output_path}\n")

    # Save sorted results to an HTML file
    sorted_df = output_df.sort_values(by="distance_to_0000").reset_index(drop=True)
    html_output_file = f"{shape_name}_sorted_by_distance_pca-umap_{pca_dim}-{umap_dim}_{CURRENT_DATETIME}.html"
    ...
    <body>
    <h1>Sorted Results by Distance for {shape_name}, PCA -> UMAP, {pca_dim} -> {umap_dim} dimensions</h1>
...
for shape_folder in SELECTED_SHAPES:
    ...
    if os.path.isdir(shape_path):
        perform_pca_umap_on_shape(shape_path, PCA_OUTPUT_DIM, UMAP_OUTPUT_DIM)
