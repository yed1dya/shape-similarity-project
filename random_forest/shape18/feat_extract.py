...
# -------------- CONFIG -------------------
INPUT_CSV = "shape18_contours_20250724_115546.csv"
OUTPUT_CSV = f"shape18_features_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
IMAGE_SIZE = (400, 400)  # modify if needed
DOWNSAMPLED_POINTS = 64
NN_K = 4
ASSIGNMENT_RADIUS = 30  # for feature 27
...
def extract_features_for_image(image_id, contours, image_shape, grouped, ref_centroids):
    ...
    try:
        ...
        hull_area = cv2.contourArea(hull)
        features["area_ratio"] = union_area / (hull_area + 1e-6)
